...
function spawnCreeps(spawn) {
    ...
    if (extenCount < 5) {
        ...
        bodyBlueprint = {
            ...
            builder: Array(3).fill(WORK).concat(Array(3).fill(CARRY), Array(3).fill(MOVE)),
            repairer: Array(2).fill(WORK).concat(Array(4).fill(CARRY), Array(4).fill(MOVE)),
            miner: Array(7).fill(WORK).concat(Array(1).fill(MOVE)),
            carrier: Array(7).fill(CARRY).concat(Array(4).fill(MOVE))
        };
        roleTargets = {
            harvester: 2,
        ...
        bodyBlueprint = {
            harvester: Array(0).fill(WORK).concat(Array(6).fill(CARRY), Array(3).fill(MOVE)),
            upgrader: Array(6).fill(WORK).concat(Array(1).fill(CARRY), Array(2).fill(MOVE)),
            builder: balancedBody,
            repairer: balancedBody,
            miner: Array(6).fill(WORK).concat(Array(2).fill(MOVE)),
            carrier: carrierBody,
            b_carrier: carrierBody,
            ldminer: Array(2).fill(WORK).concat(Array(8).fill(CARRY), Array(8).fill(MOVE)),
            scout: [MOVE, MOVE],
            selfwdis: balancedBody
        };
        roleTargets = {
            harvester: 1,
            miner: 2,
    ...
    for (const role in roleTargets) {
        if (roleCounts[role] < roleTargets[role]) {
            ...
            const body_energy = calculateCost(body)
            if (spawn.room.energyAvailable >= body_energy) {
                const name = `${role}_${Game.time}`;
                if (spawn.spawnCreep(body, name, { memory: { role } }) === OK && role === 'ldminer') {
                    Memory.longdismine.count += body_energy;
    ...
    checkMinerHealth(spawn, room, bodyBlueprint['miner']); // Call the new function
}

/**
 * Checks miner health and spawns a new miner if a miner's health is below a threshold.
 * @param {StructureSpawn} spawn - The spawn to use for creating the new miner.
 * @param {Room} room - The room the spawn is in.
 * @param {BodyPartConstant[]} minerBody - The body blueprint for the miner.
 */
function checkMinerHealth(spawn, room, minerBody) {
    const miners = _.filter(Game.creeps, c => c.memory.role === 'miner' && c.room.name === room.name);
    if (miners.length === 0) return; // No miners, no need to check.

    // 计算creeps的生产时间
    let creepBuildTime = 30;


    for (const miner of miners) {
        if (miner.ticksToLive == creepBuildTime) {
            const name = `miner_${Game.time}`;
            const body_energy = calculateCost(minerBody);
            if (spawn.room.energyAvailable >= body_energy) {
                if (spawn.spawnCreep(minerBody, name, { memory: { role: 'miner' } }) === OK) {
                    console.log(`🧬 Spawning replacement miner: ${name}`);
                    break; // Spawn only one replacement per tick
                }
            } else {
                console.log(`⚠️  Not enough energy to spawn replacement miner: ${name}`);
            }
        }
    }
}
